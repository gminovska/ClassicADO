...
public class AuthorsAndBooks
{
    ...
    public DataTable GetAuthors()
    {
        ...
        try
        {
            ...
            tbl = ProcessQuery(cmd);//Why not ReadData?
        }
        catch(Exception ex)
        {
    ...
    {
        string sql = "SELECT B.BookTitle, B.BookEntryDate, B.BookISBN FROM book B INNER JOIN authorbook AB ON B.bookkey= AB.bookkey WHERE AB.authorkey = @authorkey ";
        SqlCommand cmd = new SqlCommand(sql, connect);
    ...
    private DataTable ProcessQuery(SqlCommand cmd)
    {
       //This method processes the queries
        ...
        {
            connect.Close();//close connection (exceptions as well)
        }
        return table;
    }
}